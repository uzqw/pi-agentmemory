...
// Client-side cap so session_start / before_agent_start cannot wait on a busy
// iii worker (compress LLM timeout is 120s). Override per call if needed.
...
export default function agentmemoryExtension(pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    // Fire-and-forget: session startup must never wait on agentmemory, even
